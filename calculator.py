...
class calculator():  #calculator object initialization
    ...
    def insert_value(self, value): # inserting a number on the screen
        self.result = False
...
DotBtn = Button(calc,pady = 1, bd = 4, font = ('arial', 20, 'bold'), width = 6, height = 2, text = '.',bg = 'powder blue', command = lambda: added_value.insert_value('.')).grid(row = 5, column = 1)
...
btnEq = Button(calc, pady = 1, bd = 4, font = ('arial', 20, 'bold'), height = 2, width = 6, text = '=', bg = 'red', command = added_value.sum_of_total).grid(row = 5, column = 3)
root.mainloop()
